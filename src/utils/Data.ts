...
export const fetchPlayUrlPromise = async (bvid: string, cid: string) => {
...
export const fetchLRC = async (name: string, setLyric: Function, setSongTitle: Function) => {
	...
	try {
		...
		const text: string = await lrc.text()
...
export const fetchVideoInfo = async (bvid: string) => {
	console.info('calling fetchVideoInfo')
	...
	try {
		...
		const v = new VideoInfo(
			...
			data.pages.map((s: any) => {
				return { bvid: bvid, part: s.part, cid: s.cid }
			})
		)
		return v
	} catch (error) {
...
export const fetchBiliSeriesInfo = async (mid: string, sid: string) => {
	console.info('calling fetchBiliSeriesInfo')
	let page = 0
	let res = await fetch(
...
export const fetchBiliColleList = async (mid: string, sid: string, favList: string[] = []) => {
	...
	const BVidPromises: Promise<VideoInfo>[] = []
	const pagesPromises = [res]

	for (let page = 2; page <= totalPagesRequired; page++) {
		pagesPromises.push(
			await fetch(
				URL_BILICOLLE_INFO.replace('{mid}', mid)
					.replace('{sid}', sid)
					.replace('{pn}', page + '')
			)
		)
	...
	let videoInfos: VideoInfo[] = []
	await Promise.all(pagesPromises).then(async function (v) {
		for (let index = 0, n = v.length; index < n; index++) {
			await v[index].json().then((js) =>
				js.data.archives.map((m: any) => {
					if (!favList.includes(m.bvid)) {
						BVidPromises.push(fetchVideoInfo(m.bvid) as Promise<VideoInfo>)
					}
				})
			)
		}
...
export const fetchFavList = async (mid: string) => {
	console.info('calling fetchFavList')
	...
	const BVidPromises = data.medias.map((m: any) => fetchVideoInfo(m.bvid))
	const pagesPromises = []
	...
	await Promise.all(pagesPromises).then(async function (v) {
		...
		for (let index = 0; index < v.length; index++) {
			await v[index]
				...
				.then(
					(js) =>
						js.data.has_more && js.data.medias.map((m: any) => BVidPromises.push(fetchVideoInfo(m.bvid)))
				)
		}
...
export const searchLyricOptions = async (searchKey: string, setOptions: Function) => {
	console.info('calling searchLyricOptions')
	if (searchKey == '') {
		setOptions([])
		return
	}
	...
	const slimData = data.map((s: any, v: string) => {
		return { key: s.mid, songMid: s.mid, label: v + '. ' + s.name + ' / ' + s.singer }
	})

	if (slimData.length) setOptions(slimData)
	else searchLyricOptionsFallBack(searchKey, setOptions)
}

export const searchLyricOptionsFallBack = async (searchKey: string, setOptions: Function) => {
	console.info('calling searchLyricOptions')
	if (searchKey == '') {
		setOptions([])
		return
	}
	console.info(`calling searchLyricOptionsFallBack: ${searchKey}`)
	const API = getQQSearchAPI(searchKey)
	const res = await fetch(API.src, API.params)
	const json = await res.json()
	console.debug(json)
	const data = json.req.data.body.song.list
	const slimData = data.map((s: any, v: any) => ({
		key: s.mid,
		songMid: s.mid,
...
export const searchLyric = async (searchMID: string, setLyric: Function) => {
	console.info('calling searchLyric')
